...
<div class="login-box">
  ...
  <form>
    ...
    <div class="user-box">
      ...
      <br>
      <br>
             <input type="submit" name="sub" value="Login" style="color: white;border: 1px solid #e4e4e4;padding: 8px;border-radius: 3px;cursor: pointer; background-color:0081B4; border: none;"><br><br>
           <a href="fpasswor.py">forgot password</a>
  </form>
...
cur = conn.cursor()
if psub != None:
    q = """select id,student_id,password from student where student_id='%s' and password='%s'""" % (pstudent_id, ppass)
    cur.execute(q)
    f = cur.fetchone()
    if f != None:
        print("""
        <script>
        location.href="studentdash.py?id=%s";
        </script>
        """ % (f[0]))
    else:
        print("""
