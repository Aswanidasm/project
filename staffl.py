  ...
  <form>
    ...
    <div class="user-box">
      ...
      <br>
        ...
        <a href="forgotpassword.py">forgot password</a>
...
cur = conn.cursor()
if psub != None:
    q = """select id,staff_id,password from staff where staff_id='%s' and password='%s'""" % (pstaff_id, ppass)
    cur.execute(q)
    f = cur.fetchone()
    if f != None:
        print("""
        <script>
        location.href="stadash.py?id=%s";
        </script>
        """ % (f[0]))
    else:
        print("""
